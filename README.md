...
| **Flow** | 多个技能可能参与，事实范畴、顺序与必要性不明 | [`Vibe-Flow`](skills/vibe-flow/) | 每个幸存技能都有独特价值；分歧成为证据任务；一条或多条主要矛盾线及继续条件可见 |
...
用于“我有很多技能，但不知道该用谁、怎么组合、先做什么”的场景。事实、因果或预测存在高影响分歧时，它先根据上下文组织最小范畴专家团，把分歧转化为证据任务；随后对候选技能进行对抗式审理，再用华罗庚统筹法组织一条或多条主要矛盾线、时差、资源、并行支线与最少闸门。
